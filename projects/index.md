...
<ul class="funder-split tex2jax_ignore">
...
上列金額為<strong>核定經費</strong>：國科會與國家太空中心為計畫核定額，UAAT–TAMUS 為本實驗室以共同主持人身分承接之本校子計畫分配額。多年期計畫依年度階段分列並各自結案，故同一計畫可能同時有已結案與執行中之階段；{{ programs.size }} 項計畫共 {{ grants.size }} 筆補助紀錄，各階段均標示國科會計畫編號以供查證。教育部教學計畫性質不同，另列於下方且不計入上列總額。
<br>
Figures are <strong>approved budgets</strong>: for the NSTC and TASA the project-level award, and for UAAT–TAMUS the allocation to NTUT's Subproject 3, which this lab holds as Co-PI. Multi-year programs are funded one annual phase at a time and each phase closes on its own term, so a single program may show a completed phase alongside a running one — {{ programs.size }} programs account for {{ grants.size }} funding records, each carrying its NSTC project number for verification. MOE teaching grants are a different kind of work, listed separately below and excluded from this total. NSTC entries reconciled against the
...
<h3 class="kind-heading tex2jax_ignore" id="kind-{{ kind.id }}">{% include icon.html icon=kind.icon %}{{ kind.label }} · {{ kind.label-en }}<span class="kind-count">{{ kind_programs.size }} program{% if kind_programs.size != 1 %}s{% endif %}{% if in_kind.size != kind_programs.size %} · {{ in_kind.size }} records{% endif %} · NT${% include number.html value=kind_total %}</span></h3>
...
{% if leftovers > 0 %}
<h3 class="kind-heading tex2jax_ignore" id="kind-other">{% include icon.html icon="fa-solid fa-folder-open" %}其他 · Other<span class="kind-count">{{ leftovers }} record{% if leftovers != 1 %}s{% endif %}</span></h3>
<div class="grant-list" data-search-group>
...
<h3 class="kind-heading tex2jax_ignore" id="kind-teaching">{% include icon.html icon="fa-solid fa-chalkboard-user" %}教學計畫 · Teaching Grants<span class="kind-count">{{ teaching_programs.size }} program{% if teaching_programs.size != 1 %}s{% endif %}{% if teaching.size != teaching_programs.size %} · {{ teaching.size }} records{% endif %}</span></h3>
<div class="grant-list" data-search-group>
